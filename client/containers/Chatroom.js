							...
							<div style={chatboxStyle}>
								{this.props.messages.map(message => {
									if (message.sender === this.state.selected_user) {
										return (
											<li style={messageFloatingRight}>{message.message}</li>
										)
									} else if (message.receiver === this.state.selected_user) {
										return (
											<li style={messageFloatingLeft}>{message.message}</li>
										)
									}
								})}
							</div>
...
function mapDispatchToProps(dispatch) {
	return {
		...
		getMessageForThisUser: function() {
			dispatch(getAllMessages())
		},
		addNewMessage: function(message) {
			dispatch(addNewMessageAction(message))
		}
	}
}
